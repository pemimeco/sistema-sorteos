...
document.querySelector('#btnLimiteR').addEventListener('click', () => {
    ...
    console.log('cant1', cant1, 'cant2', cant2)
    if (cant1 != 0) {
...
function RepLimite() {
    setTimeout(() => {
        url.apiAreasxEstRep(codigo, document.querySelector('#filtro').value)
            .then((res) => {
                let data = res.areasRep.data
                for (let index = 0; index < data.length; index++) {
                    // const element = data[index];
                    document.querySelector('#area1').innerHTML = data[0].area
                    document.querySelector('#area2').innerHTML = data[1].area
                ...
                console.log(data[0].area, data[1].area)
            });
    }, 100);
}
